...
# automatically generated to add the LaTeX contrib: tabularray
# to kerTeX.
#
...
for suffix in bst bib; do
	...
	pkg_lstree | sed "/\.$suffix\$/!d" | while read file; do
		if test $has_some = "NO"; then
			mkdir -p $TMPDIR/lib/bibtex/tabularray/$suffix
			ed -s $PKG_CID <<EOT
/^KXPATH:/a
	bibtex tabularray/$suffix
.
w
q
EOT
		...
		mv $file $TMPDIR/lib/bibtex/tabularray/$suffix/$file
	done
done
...
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/tabularray.zip
END:
END_CID
